...
def download_table_as_xml(table_to_download, timestamp):
    ...
    #if timestamp:
    #    outfilename = table_to_download + ' ' + strftime("%Y-%m-%d %H_%M_%S") + '.xml.gz'
    #else:
...
if args.backup:
    if 'instance' not in config or 'username' not in config or 'password' not in config or 'theTables' not in config:
        print('Please ensure settings are available in ', configFilePath)
        sys.exit(1)
    theTables = config['theTables']

    backupdirectory = 'snbackup_' + config['instance'].split('.')[0] + '_' + strftime("%Y_%m_%d-%H_%M_%S")
    #backupdirectory = 'snbackup_' + strftime("%Y_%m_%d-%H_%M_%S")
    print(backupdirectory)
    os.mkdir(backupdirectory)
    os.chdir(backupdirectory)

    for table in theTables.split(','):
        print(table)
        download_table_as_xml(table, args.timestamp)

    os.chdir("..")
    zf = zipfile.ZipFile(backupdirectory+'.zip', mode='w')
    try:
        print('Zipping Folder')
        zipdir(backupdirectory+'/', zf)
    finally:
        print('Zip Succesful')
        zf.close()
